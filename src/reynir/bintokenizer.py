            ...
            if wm:
                ...
                for sl, ix in state[wm]:
                    if not sl:
                        ...
                        yield TOK.Word(
                            w, map(BIN_Meaning._make, StaticPhrases.get_meaning(ix))
                        )
            ...
            if wm:
                ...
                for sl, ix in pdict[wm]:
                    if not sl:
                        ...
                        yield TOK.Word(
                            token.txt,
                            map(BIN_Meaning._make, StaticPhrases.get_meaning(ix)),
                        )
                        newstate = defaultdict(list)
                        token = None
                        break
        ...
        while True:
            ...
            if (
                ...
                next_token = TOK.Word(
                    token.txt,
                    None,
                    error=compound_error(2, token.error, next_token.error),
                )
                token = next_token
                continue
            ...
            if (token.txt, next_token.txt) in SPLIT_COMPOUNDS:
                token = TOK.Word(
                    token.txt + next_token.txt,
                    None,
                    error=compound_error(token.error, 5, next_token.error),
                )
                continue
