...
from typing import (
    Any, Iterable, Iterator, Optional, Union, Callable,
...
from .bintokenizer import tokenize as bin_tokenize, Tok
...
class _Sentence:
    ...
    def __init__(self, job: "_Job", s: List[Tok]) -> None:
        self._job = job
        # s is a token list
        self._s = s
        self._len = len(s)
...
class _Job:
    ...
    def _create_sentence(self, s: List[Tok]) -> _Sentence:
...
class _Job_NP(_Job):
    ...
    def _create_sentence(self, s: List[Tok]) -> _NounPhrase:
